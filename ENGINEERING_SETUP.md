...
## Read-only staging preflight (no credential disclosure)
The branch contains `npm run test:staging:readonly`. Run this ONLY after creating a separately authorized HTTPS staging URL with the existing Firebase project's public Web App settings and the precise database ID recorded above. Set `STAGING_BASE_URL` in the test process. The script **rejects the original production host** and checks JSON health and 401 for anonymous private/admin APIs. For deeper read-only checks supply short-lived test-account Firebase ID tokens securely as `STAGING_STUDENT_ID_TOKEN` and `STAGING_OWNER_ID_TOKEN`; never paste ID tokens into chat, commit them, or include them in CLI arguments. It checks student 403 and owner project/database metadata access. No student documents are written by this script. It does not establish client Firestore security rules, payment functionality, or full browser user journeys. Never enable GitHub Actions production deploy from this branch merely because this script succeeds.
